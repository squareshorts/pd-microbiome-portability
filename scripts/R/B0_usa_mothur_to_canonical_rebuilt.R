...
raw_dir   <- file.path(repo_root, "data/raw/usa_mothur/Estados Unidos")
out_dir   <- file.path(repo_root, "data/processed/usa")
...
# ---- Read PD vs Control ----
pd_counts  <- read_shared_mothur(file.path(raw_dir, "PD.opti_mcc.shared"))
ctl_counts <- read_shared_mothur(file.path(raw_dir, "USA_CONTROL.opti_mcc.shared"))
...
male_ids   <- read_shared_mothur(file.path(raw_dir, "USA_MALE.opti_mcc.shared"))$SampleID
female_ids <- read_shared_mothur(file.path(raw_dir, "USA_FEMALE.opti_mcc.shared"))$SampleID
...
if (any(is.na(counts$Sex))) {
  ...
  stop(paste("USA: missing Sex for", nmiss, "samples."))
}

# ---- Taxonomy ----
tax <- fread(file.path(raw_dir, "PD.opti_mcc.0.03.cons.taxonomy"))
...
# ---- Save ----
fwrite(wide, file.path(out_dir, "usa_genus_counts_rebuilt.csv"))
fwrite(wide[, .(SampleID, PD, Sex)], file.path(out_dir, "usa_metadata_rebuilt.csv"))
