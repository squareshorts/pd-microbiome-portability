# scripts/R/paper2/C26_within_cohort_logreg.R
cat("C26 WITHIN-COHORT LOGREG STARTED\n")
cat("WD:", getwd(), "\n")
...
for (coh in cohorts) {
  ...
  for (r in 1:R) {
    folds <- make_strat_folds(yc, K=K, seed=100 + r)
    for (k in 1:K) {
      test_idx <- folds[[k]]
      ...
      # CV for lambda within training only
      set.seed(1)
      cvfit <- cv.glmnet(
  ...
  fwrite(fold_tab, file.path("results/paper2/within", paste0("within_logreg_", coh, "_folds.csv")))

  out_rows[[length(out_rows)+1]] <- fold_tab[, .(
    model="within_logreg_ridge",
    cohort=coh,
    n=n,
    prev=prev,
...
fwrite(sum_tab, "results/paper2/table_within_cohort_logreg_summary.csv")

cat("\nSaved: results/paper2/table_within_cohort_logreg_summary.csv\n")
cat("C26 WITHIN-COHORT LOGREG DONE\n")